...
import { RegistroDevOpExamenDTO } from 'src/app/Models/ExamenDTO';
...
export class ModoEntrenamientoComponent implements OnInit {
  ...
  RegistrarExamen(){
    if(this.userForm.valid){
      ...
      this.RegistrarExamenEnvio.idSimuladorDevOpModo=2,
    ...
    this._ExamenService.ListaExamenesIncompletos().subscribe({
      next:(x)=>{
        ...
        this.SimulacionesIncompletas.forEach((y:any)=>{
          if(y.idEstadoExamen!=3 && y.idSimuladorDevOpModo==2){
